...
SingleLineComment
  = "//" content:(!LineTerminator SourceCharacter)*
...
Literal
  ...
  / value:PhpLiteral        { return { type: "PhpLiteral",     value: value } }
...
PhpLiteral
  = "<?" path:PhpLiteralCharacters "?>" {
    return path.trim ();
  }
  
PhpLiteralCharacters
  = chars:PhpLiteralCharacter+ { return chars.join(""); }
  
PhpLiteralCharacter
  = !('?') char_:SourceCharacter { return char_;           }
  / temp:("?"!">")               { return temp.join ("");  }
...
PrimaryExpression
  ...
  / ArrayLiteral
  / ObjectLiteral
  / "(" __ expression:Expression __ ")" { return { type: "ParenthesizedExpression", value: expression }; }
...
AssignmentExpression "assigment expression"
...
AssignmentOperator "assigment operator"
  = "=" (!"=") { return "="; }
  / "*="
  / "/="
  / "%="
...
ExpressionStatement
  = !("{" / FunctionToken) expression:Expression EOS { return { type: "ExpressionStatement", value: expression }; }
...
TryStatement
  = TryToken __ block:Statement __ catch_:Catch __ finally_:Finally {
      return {
        type:      "TryStatement",
        block:     block,
        "catch":   catch_,
  ...
  / TryToken __ block:Statement __ catch_:Catch {
      return {
        type:      "TryStatement",
        block:     block,
        "catch":   catch_,
  ...
  / TryToken __ block:Statement __ finally_:Finally {
      return {
        type:      "TryStatement",
        block:     block,
        "catch":   null,
...
Catch
  = CatchToken __ "(" __ identifier:Identifier __ ")" __ block:Statement {
      return {
        type:       "Catch",
        identifier: identifier,
        block:      block
...
Finally
  = FinallyToken __ block:Statement {
      return {
        type:  "Finally",
        block: block
      };
...
FormalParameterList
  = head:IdentifierWithDefault tail:(__ "," __ IdentifierWithDefault)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
__ns
  = (WhiteSpace / LineTerminatorSequence / CommentNs)*
...
SingleLineCommentNs
  = !SpecialCommentStart "//" (!LineTerminator SourceCharacter)*
    
SpecialComment
  = begin:SpecialCommentStart content:SpecialCommentPart* end:SpecialCommentEnd {
      if (begin != end)
        throw 'Wrong special comment: "' + begin + '" != "' + end + '".';
      return {
        type:       "SpecialComment",
        tag:        begin,
        value:      content
...
SpecialCommentStart
  = "// ==" name:IdentifierName "==" EOLWith { return name.toLowerCase () }
  
SpecialCommentEnd
  = "// ==/" name:IdentifierName "==" WhiteSpace* { return name.toLowerCase () }
    
EOLWith
  = WhiteSpace* LineTerminator (WhiteSpace / LineTerminatorSequence)*
  
SpecialCommentKeyChar
  = !WhiteSpace c:SourceCharacter { return c }
  
SpecialCommentValueChar
  = !LineTerminator c:SourceCharacter { return c }
  
SpecialCommentPart
  = "// @" key:SpecialCommentKeyChar* WhiteSpace+ value:SpecialCommentValueChar+ EOLWith {
    return { key: key.join ('').trim (), value: value.join ('').trim () }
  }
  / !"// ==/" (!LineTerminatorSequence .)* EOLWith {
    return { key: null, value: null }
  }

Program
  = __ns elements:SourceElements? __ns {
      return {
        type:     "Program",
        elements: elements !== "" ? elements : []
      };
...
/* ===== EXT STUFF ===== */
Module
  = (ModuleToken / PackageToken) __ identifier:Identifier __ "{" __ elements:ModuleBody __ "}" {
      return {
        type:        "Module",
        identifier:  identifier,
        elements:    elements
...
ModuleBody
  = elements:ModuleSourceElements? { return elements !== "" ? elements : []; }
  
ModuleSourceElements
  = head:ModuleSourceElement tail:(__ ModuleSourceElement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][1]);
      }
...
ExportFunctionDeclaration
  = ExportToken __ func:FunctionDeclaration {
    func.exportFlag = true;
    return func;
  }
  ...
  = HtmlToken __ "{{" data:(!"}}" .)* "}}" {
    function parse (s){      
      try {
        ...
        do {
          var resume = false, first;
          s = s.replace (/(\n\s+)([\w$]+)\s*:/i, function (all, $1, $2, index){
            if (!first)
              first = $1;
            var cut_to = s.indexOf ($1, index + 1);
            while (cut_to !== -1 && ' \t'.indexOf (s [cut_to + $1.length]) !== -1)
              cut_to = s.indexOf ($1, cut_to + 1);
            
            data [$2] = s.substring (index + all.length, cut_to === -1 ? s.length : cut_to).split (first).join ('\n');
            resume = true;
            return '';
          });
        } while (resume);
        ...
        console.error ('Html content has wrong format: ' + e);
      }
    }
    
    return {
      type: "HtmlStatement",
      data: parse (data.map (function (a){ return a [1] }).join (''))
    }
  }
  
HtmlDeclarationList
  = head:HtmlDeclaration tail:(__ "," __ HtmlDeclaration)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
HtmlEntry
  = "<" Identifier HtmlAttributes ">" (__ HtmlEntries __)? "</" Identifier ">"
 // / "<" Identifier HtmlAttributes "/>"        // { console.log (column); return "" }
  / (!"<" !">" SourceCharacter)+
    
HtmlDeclaration
  = name:Identifier __ ":" __ data:HtmlEntry {
      return {
        type: "HtmlDeclaration",
        name: name,
        data: (function (a){ 
          function s (o){
            if (typeof o === 'string')
...
ExportVariableStatement
  = ExportToken __ vars:VariableStatement {
    vars.exportFlag = true;
    return vars;
  }
    
ImportStatement
  = ImportToken __ declarations:ImportDeclarationList EOS {
      return {
...
ImportDeclarationList
  = head:ImportIdentifier tail:(__ "," __ ImportIdentifier)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][3]);
      }
...
LambdaExpression
  = LambdaToken __ "(" __ params:FormalParameterList? __ ")" __ element:NoCommaStatement {
      return {
        type:     "Lambda",
  ...
  / LambdaToken __ element:NoCommaStatement {
      return {
        type:     "Lambda",
        params:   [],
        element:  element
  ...
  /* LambdaToken __ params:FormalParameterList __ ":" __ element:NoCommaStatement {
      return {
        type:     "Lambda",
        params:   typeof params !== "undefined" && params !== "" ? params : [],
        element:  element
...
NoCommaStatement
  ...
  / EmptyStatement
  / IfNoCommaStatement
  / IterationNoCommaStatement
  / ContinueStatement
  / BreakStatement
  / ReturnNoCommaStatement
  / WithStatement
  / LabelledStatement
  / SwitchStatement
  / ThrowStatement
  ...
  / FunctionDeclaration
  / FunctionExpression
  / ExpressionNoCommaStatement

ExpressionNoCommaStatement
  = !FunctionToken expression:ExpressionNoComma { return { type: "ExpressionStatement", value: expression }; }
...
IterationNoCommaStatement
  = DoWhileStatement
  / WhileNoCommaStatement
  / ForNoCommaStatement
  / ForInNoCommaStatement
  
IfNoCommaStatement
  = IfToken __
    "(" __ condition:Expression __ ")" __
    ifStatement:NoCommaStatement
    elseStatement:(__ ElseToken __ NoCommaStatement)? {
      return {
        type:          "IfStatement",
        condition:     condition,
        ifStatement:   ifStatement,
...
ReturnNoCommaStatement
  = ReturnToken _
    value:(
        expression:AssignmentExpression EOS? { return expression; }
      / EOSNoLineTerminator       { return ""; }
    ) {
      return {
        type:  "ReturnStatement",
...
WhileNoCommaStatement
  = WhileToken __ "(" __ condition:Expression __ ")" __ statement:NoCommaStatement {
      return {
        type: "WhileStatement",
        condition: condition,
        statement: statement
...
AllStringCharacters
  = chars:AllStringCharacter+    { return chars.join(""); }
