...
# Изменения синтаксиса, новые операторы и всё такое
	...
	*/
	...
	/*
		Небезынтересная деталь:  если парсер  после ключевого слова  «function», 
		имени функции  или списка аргументов  найдёт фигурную скобку,  сперва он 
		попробует распарсить содержимое как объект. Вероятнее всего, парсер нат-
		кнётся на ошибку - в таком случае он уже будет парсить функцию.

		Эта особенность может стать причиной неверного парсинга  JavaScript-кода 
		— но только если у вас есть функция, которая выглядит как объект.
	*/
	...
	*/
	...
	/*
		...
		Они совпадают с функциями во всём  за исключением лишь одной мелочи - не
		могут иметь имени, а потому не могут определять новые функции.
...
### Обычные классы
	...
	/*
		...
		скобки - иначе распарсить код часто будет невозможно. Кроме того, если у
		них нет аргументов, они не получат стандартный аргумент «arg».
