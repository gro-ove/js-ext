// ==Jsx==
// @include utils/object.extend
// @include utils/string.format
// ==/Jsx==
...
module App {
	import Node, Prework, Parser, Converter, Format, JsCompress;
	...
	function loadAndParse (file, top){
		...
		var result = {
				...
				apply:      lambda (other) join (this, other)
			}, 
			childs = [],
			flags = {};
		...
		for (var i = 0; i < formatted.jsx.length; i ++)
			switch (formatted.jsx [i].key){
				...
				case 'target':
					...
					break;
				default:
					Node.fatalError ('Undefined instruction: "{0}".'.format (formatted.jsx [i].key));
			}
