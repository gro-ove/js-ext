...
fatalError ("Please, install `%%` for %%.".format (what, purpose || "work"), "> npm install -g `%%`.".format (what));
}
};
}
...
return arg.value;
},"UserScript":function (arg){
return commentParse (additional.userscript, arg.value) && "";
},"Jsx":function (arg){
return commentParse (jsx, arg.value) && "";
...
__m ("App", function (){
var Format, Node, Parser, Converter, JsCompress;
var moduleSystem;
...
var content = Node.readFile (file), parsed = Parser.work (content), converted = Converter.work (parsed), formatted = Format.work (converted.tree);
...
break;
default:Node.fatalError ("Undefined instruction: \"%%\"".format (formatted.jsx [i].key));
}
if (flags.compress)
...
return ["Format","Node","Parser","Converter","JsCompress"];
},"__si":function (){
Format = arguments [0];
Node = arguments [1];
Parser = arguments [2];
Converter = arguments [3];
JsCompress = arguments [4];
},"__in":function (){
