#!/usr/bin/env node
!function(){function get(id){if(storage[id])return storage[id];throw"Not found: "+id}function init(id){var module=get(id);module.__z||(module.__g=1,module.__gi().forEach(function(other){if("@"!=other[0]){if(get(other).__g)throw"Cycle: "+id+", "+a;get(other).__z||init(other)}})),module.__in(),module.init&&module.init(),module.__z=1,delete module.__g}var storage={},root=GLOBAL;root.__m=function(id,module){if(module)storage[id]=module();else{for(id in storage){var module=get(id);module.__si.apply(module,module.__gi().map(function(id){return get("@"==id[0]?id.slice(1):id)}))}for(id in storage)init(id);delete root.__m}}}();
...
var result = [], modules = data.modules && ModulesCode.get ({"target":data.additional.target && data.additional.target.value});
...
var str = jsCompressor ().OutputStream ({"comments":/^!<\?/}), prs = jsCompressor ().parse (code, {"filename":filename}), cmp = jsCompressor ().Compressor (max ? {"sequences":true,"properties":true,"dead_code":true,"drop_debugger":true,"unsafe":true,"conditionals":true,"comparisons":true,"evaluate":true,"booleans":true,"loops":true,"unused":false,"hoist_funs":true,"hoist_vars":true,"if_return":true,"join_vars":true,"cascade":true,"side_effects":true,"global_defs":{}} : {"unused":false,"warnings":Node.args ().o}), trn = prs.transform (cmp);
trn.print (str);
return str.toString ();
}
...
return {"work":work,"max":max,"__gi":function (){
return ["Node"];
},"__si":function (){
Node = arguments [0];
},"__in":function (){
...
var begin = "!function () {\n\t\t\t    var storage = {}, root = {0};\n\t\t\t    \n\t\t\t    function get (id) {\n\t\t\t        if (storage [id]) \n\t\t\t        \treturn storage [id];\n\t\t\t        throw \"Not found: \" + id;\n\t\t\t    }\n\n\t\t\t    function init (id) {\n\t\t\t        var module = get (id);\n\t\t\t        if (!module.__z){\n\t\t\t        \tmodule.__g = 1;\n\t\t\t        \tmodule.__gi ().forEach (function (other){\n\t\t\t\t            if (\"@\" != other [0]) {\n\t\t\t\t                if (get (other).__g) \n\t\t\t\t                \tthrow \"Cycle: \" + id + \", \" + a;\n\t\t\t\t                if (!get (other).__z) \n\t\t\t\t                \tinit (other);\n\t\t\t\t            }\n\t\t\t\t        });\n\t\t\t        }\n\t\t\t        \n\t\t\t        module.__in();\n\t\t\t        if (module.init) \n\t\t\t        \tmodule.init ();\n\t\t\t        module.__z = 1;\n\t\t\t        delete module.__g;\n\t\t\t    }\n\t\t\t    \n\t\t\t    root.__m = function (id, module) {\n\t\t\t        if (module){\n\t\t\t        \tstorage [id] = module ();\n\t\t\t        } else {\n\t\t\t            for (id in storage){\n\t\t\t            \tvar module = get (id);\n\t\t\t            \tmodule.__si.apply (module, module.__gi ().map (function (id){\n\t\t\t                \treturn get (\"@\" == id [0] ? id.slice (1) : id)\n\t\t\t            \t}));\n\t\t\t            }\n\t\t\t            \n\t\t\t            for (id in storage)\n\t\t\t            \tinit (id);\n\t\t\t            \n\t\t\t            delete root.__m\n\t\t\t        }\n\t\t\t    }\n\t\t\t}();".format (roots [target (options.target)]), end = options.target == "web:onload" ? "'u' != (typeof window)[0] && 'u' != (typeof document)[0] && !/loaded|complete/.test(document.readyState) ? window.addEventListener('load', __m) : __m()" : "__m()";
return {"begin":JsCompress.max (begin),"end":JsCompress.max (end)};
}
return {"get":get,"__gi":function (){
return ["JsCompress"];
},"__si":function (){
JsCompress = arguments [0];
},"__in":function (){
roots = {"node":"GLOBAL","web":"window","auto":"\"u\" != (typeof window)[0] && window || GLOBAL"};
}};
});
__m ("Parser", function (){
var Node, JsCompress;
